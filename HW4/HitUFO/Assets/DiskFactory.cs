...
public class DiskFactory : MonoBehaviour
{
    ...
    public GameObject GetDisk(int round)
    {
        ...
        if (disk_prefab == null)
        {
            if (tag == "UFO1")
            {
                disk_prefab = Instantiate(Resources.Load<GameObject>("UFO1"), new Vector3(0, start_y, 0), Quaternion.identity);
            }
            else if (tag == "UFO2")
            {
                disk_prefab = Instantiate(Resources.Load<GameObject>("UFO2"), new Vector3(0, start_y, 0), Quaternion.identity);
            }
            else
            {
                disk_prefab = Instantiate(Resources.Load<GameObject>("UFO3"), new Vector3(0, start_y, 0), Quaternion.identity);
            }
            //给新实例化的飞碟赋予其他属性
